<script setup>
  ...
  const cosumidor_final_id = import.meta.env.VITE_CONSUMIDOR_FINAL_ID
  ...
  const getProforma = async () => {
    if( route.params.proforma_id !== '' ){
      const { data } = await api.get(`/invoices/filterInvoice/${ route.params.proforma_id }`);
      ...
      nombre_proforma.value        = data.name_proforma
      ...
      data.invoiceToProduct.forEach(product => {
        ...
        product.product_id.pvp = parseFloat(product.v_total) / parseInt(product.cantidad);
  ...
  const agregarProduct = ( product ) => {
    agregarAndValidarStock( product, 'proforma' )
  ...
  const validarCampos = (tipo_documento) => {
      ...
      rows.value.forEach((row, index) => {
        ...
        if (row.cantidad > row.stock && row.tipo != 'Servicio' && tipo_documento == 'EMISION') {
          existError = true;
  ...
  const onSubmit = async ( tipo, edit = false ) => {
    ...
    rows.value.map( row => {
      ...
      row.v_total = formatearNumero(row.v_total)
    });
    ...
    <div class="row q-col-gutter-lg flex items-center">
      ...
      <div
        ...
        <q-breadcrumbs class="row q-mr-xs"
          ...
          <q-breadcrumbs-el label="Proformas" icon="payments" to="/proformas" />
          <q-breadcrumbs-el label="Proforma" icon="add_circle" />
        </q-breadcrumbs>
      </div>

      <div class="col-xs-12 col-md-6"
        :class="[ $q.screen.width < 1022 ? 'text-center q-pt-sm' : 'text-right q-pt-sm']">
        <label class="text-h6 text-center">Nueva Proforma</label>
      </div>
    </div>
  </div>
          ...
          <div class="col-xs-12 col-sm-12 col-md-6 q-mt-md q-pl-none">
            ...
            <q-select color="orange" filled v-model="formVenta.customer_id"
              ...
              <template v-if="formVenta.customer_id && formVenta.customer_id !== 'CONSUMIDOR FINAL'" v-slot:append>
                <q-icon name="cancel"
                  @click.stop.prevent="formVenta.customer_id = cosumidor_final_id" class="cursor-pointer" />
              </template>
        ...
        <div class="row q-pt-lg q-col-gutter-md"
          ...
          <div class="col-xs-12 col-md-6 q-pl-none">
            ...
            <q-select
              ...
              @update:model-value="agregarAndValidarStock( filterByCodBarra.detalles, 'proforma')"
        ...
        <q-card class="q-mt-md">
          <q-card-section class="q-pt-none row">
            ...
            <div class="col-12 flex q-mt-md q-my-lg"
              ...
              <q-btn-dropdown v-else outline rounded
                style="color: #696cff" label="Generar"
                :class="$q.screen.width >= 1023 ? 'q-px-xl': 'full-width'">
                <q-list>
                  <q-item clickable v-close-popup @click="onSubmit('EMISION')">
                    <q-item-section>
                      <q-item-label>Emitir Factura</q-item-label>
                    </q-item-section>
                  </q-item>
